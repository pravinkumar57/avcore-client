...
export class MediasoupSocketApi implements IMediasoupApi{
    ...
    async producersStatsByStream(json:StreamKindsData):Promise<StatsOutput>{
    ...
    async tcpStreaming(json:TcpStreamingRequest):Promise<StreamingPortsData>{
        return (await this.request(ACTION.TCP_STREAMING,json) as StreamingPortsData);
    }
    ...
    async mixerPipeStart(json:MixerPipeLiveData|MixerPipeRecordingData|MixerPipeRtmpData|MixerPipeHlsData|MixerPipePortData):Promise<MixerPipeInput>{
    ...
    async clearPipeTransports(json:ClearPipeTransportsInput):Promise<void>{
        await this.request(ACTION.CLEAR_PIPE_TRANSPORTS,json);
    }
